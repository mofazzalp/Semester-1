...
while loop == 0:
    ...
    if choose == 1:
        ...
        for question_no in range(user_option):
            ...
            Count_inConnection += 1  # counting how many times math 1 questions taken. this will be use for final average
            ...
            local_correct_ans_count = 0  # reset previous / 1st attempt  answer count ,  2nd attempt will be count in 61 th line
    ...
    if choose == 2:
        ...
        for line in connection1:
            ...
            user_answer = input(f"{local_correct_ans_count}: {weekdays_count} = ")
            if user_answer == weekdays:
                correct_answers += 1
                word_questions1 += f"{local_correct_ans_count} : {weekdays_count}, = {weekdays} {SPECIAL_CHARACTER_CORRECT}\n"
            else:
                word_questions1 += f"{local_correct_ans_count} : {weekdays_count}, = {user_answer} {SPECIAL_CHARACTER_WRONG} should be {weekdays}\n"
    ...
    if Ask_again == 'no':
        ...
        loop = 2
        average_score = (correct_answers / (Count_inConnection * exam_taken_level)) * 100
        print(f"{first_name} Your average score for this attempts is {average_score}")
