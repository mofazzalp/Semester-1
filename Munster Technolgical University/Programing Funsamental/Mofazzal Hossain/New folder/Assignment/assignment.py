...
while loop == 0:
    ...
    if choose == 1:
        ...
        Count_inConnection += option  # counting how many times math 1 questions taken.
        ...
        for question_no in range(user_option):
            ...
            if user_answer == answer:
                ...
                questions1 += (f"{question_no + 1} : {calculation_question}, = {user_answer} {SPECIAL_CHARACTER_WRONG} "
                               f"should be {answer}\n")
        print('\n')
        print(questions1)
    ...
    if choose == 2:
        ...
        for line in connection1:
            ...
            if user_answer == weekdays:
                ...
                word_questions1 += (f"{local_questions_count} : {weekdays_count}, = "
                                    f"{weekdays} {SPECIAL_CHARACTER_CORRECT}\n")
            else:
                word_questions1 += (f"{local_questions_count} : {weekdays_count}, = "
                                    f"{user_answer} {SPECIAL_CHARACTER_WRONG} should be {weekdays}\n")
    ...
    Ask_again = ''
    while Ask_again != 'yes' and Ask_again != 'no':
        Ask_again = input("Please enter yes or no: ").lower()
        if Ask_again == 'no'.lower():
            loop = 0
        elif Ask_again == 'yes'.lower():
            loop = 2
            average_score = (correct_answers / Count_inConnection) * 100
            print(f"{first_name} Your average score for this attempt is {average_score:,.0f}%")
            print("Your teacher can view details in logs.txt.")
